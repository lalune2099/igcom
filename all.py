...
# def safe_mid_prices(prices, version):
#     """
#     夏令时破局版：认清 IG Europe 返回法兰克福时间 (UTC+2) 的现实，
#     先盖上德国戳，再转伦敦戳。
#     """
#     if len(prices) == 0:
#         raise Exception("Historical price data not found")
#     df = json_normalize(prices)
#     # 直接使用带有规律的 snapshotTime，无视那个被乱减 8 小时的有毒 UTC 列
#     if "snapshotTime" not in df.columns:
#         if "snapshotTimeUTC" in df.columns:
#             df["snapshotTime"] = df["snapshotTimeUTC"]    
#     df = df.set_index("snapshotTime")
#     df = df.drop(columns=["snapshotTimeUTC"], errors="ignore")
#     df.index = pd.to_datetime(df.index)
#     # ================= 核心修复 =================
#     # 既然它吐出来的是法兰克福时间，我们就实事求是，给它盖上德国（欧洲/柏林）的时区戳
#     if df.index.tz is None:
#         import pytz
#         TZ_GERMANY = pytz.timezone("Europe/Berlin")
#         df.index = df.index.tz_localize(TZ_GERMANY)
#     # 接着，从德国时间转换为伦敦时间。
#     # pytz 非常聪明，它知道现在德国是 UTC+2，伦敦是 UTC+1，会自动减去 1 个小时！
#     # 于是 18:00 就会被精准还原成 17:00！
